...
LIST_PERFORMACES_ENDPOINT = "/v0/performances/"
...
def call_api_endpoint(
    ...
    try:
        ...
        logger.error(f"HTTP status error ocurred: {e.response.text}")
        return httpx.Response(status_code=e.response.status_code,
                              content=b"API error")
    
    except httpx.RequestError as e:
        logger.error(f"Request error ocurred: {str(e)}")
        return httpx.Response(status_code=500, content=b"Network error")

    except Exception as e:
        logger.error(f"Unexpected error ocurred: {str(e)} ")
